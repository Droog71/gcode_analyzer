...
def get_gcode_info(filename):
    ...
    size = "Size: " + str(round(gcode.width, 1)) + "x " + str(round(gcode.depth, 1)) + "y " + str(round(gcode.height, 1)) + "z\n"
    filuse = "Filament used: %0.02fmm" % gcode.filament_length + "\n"
    layers = "Number of layers: %d" % gcode.layers_count + "\n"
    time = "Estimated duration: %s" % gcode.estimate_duration()[1]
    info = los + llos + pos + size + filuse + layers + time
...
# main program
window = Tk() 
window.title('G-code Analyzer') 
window.geometry("300x400")  
window.config(background = "gray")
try:
	img = PhotoImage(file='printer.png')
	Label(window, image=img).pack()
except:
	print("Missing printer.png image file.")
explore_border = LabelFrame(window, bd = 4, bg = "black")
explore_border.pack(pady = 8)
exit_border = LabelFrame(window, bd = 4, bg = "black")
exit_border.pack(pady = 8)
button_explore = Button(explore_border,text = "Browse Files", fg='black', bg='gray', command = browse_files)
